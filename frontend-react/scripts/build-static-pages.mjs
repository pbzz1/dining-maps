...
//   VITE_CONTACT_EMAIL                                            /data/ 페이지 문의처 (개인정보 문의처는 privacyPage 의 PRIVACY_EMAIL)
...
// 개인정보 처리방침 (개인정보보호법 30조, 시행령 31조의 기재 사항 전부). 수집 항목·보관 기간·
// 국외 이전은 코드에서 확인한 사실만 적는다 -- db/schema.sql 의 app_user~llm_spend,
// app/auth/kakao.py(scope), app/auth/consent.py(민감정보 별도 동의), app/recommend/personal.py·
// app/chat/service.py(프롬프트에 들어가는 값), index.html 의 GA4·Clarity,
...
function privacyPage(ctx) {
  ...
  const adsense = ADSENSE_CLIENT
    ? row(`Google LLC<br>${ext("https://policies.google.com/privacy", "처리방침·문의")}`, "미국",
      "광고 쿠키, 광고 식별자, 기기·브라우저 정보", "광고 게재(Google AdSense)와 부정 클릭 방지", "Google 정책에 따름")
    : "";
  return page({
    ...
    body: `<div class="doc">
<h1>개인정보 처리방침</h1>
<p class="lead">Dining Maps(이하 "서비스")는 ${PRIVACY_OWNER}이 개인으로 운영하는 프랜차이즈 메뉴 영양정보 서비스입니다.
서비스는 「개인정보 보호법」에 따라 이용자의 개인정보를 보호하고 관련 고충을 신속하게 처리하기 위해 이 방침을 둡니다.
로그인하지 않으면 지도·메뉴·랭킹·맞춤 추천을 쓰더라도 서비스 서버에 이용자를 알아볼 수 있는 정보를 저장하지 않습니다.
시행일 ${PRIVACY_EFFECTIVE}.</p>

<h2>1. 처리하는 개인정보의 항목과 목적</h2>
<div class="wrap"><table>
<tr><th>언제</th><th>항목</th><th>목적</th></tr>
${row("카카오 로그인(회원가입)", "카카오 회원번호, 닉네임(선택 동의), 가입·최근 로그인 시각", "회원 식별, 화면에 이름 표시")}
${row("추천 설정 저장(로그인 시)", "목표, 활동량, 한 끼 칼로리·나트륨 상한, 음료 제외 여부, 싫어하는 재료", "기기 간 설정 동기화, 맞춤 추천")}
${row("<b>신체정보 저장(별도 동의 시에만)</b>", "성별, 키, 몸무게, 나이, 알레르기", "한 끼 적정 열량 계산, 알레르기 메뉴 제외, 기기 간 동기화, 유료 요금제의 AI 맞춤 추천")}
${row("서비스 이용(로그인 시)", "메뉴 조회·저장·숨김 기록, 추천에 노출된 메뉴 목록, AI 메모리(취향을 요약한 문장)", "맞춤 추천 개선, 저장한 메뉴 모아 보기")}
${row("유료 요금제 결제", "주문번호, 요금제, 금액, 결제 키, 결제 승인 결과(결제수단 종류 등 토스페이먼츠 응답)", "이용권 지급, 환불·취소 처리, 법정 거래기록 보관")}
${row("AI 기능 이용(유료)", "AI 호출 시각·용도·사용량·비용", "요금제별 사용 한도 계산")}
${row("위치 기반 기능", "현재 위치 좌표(브라우저 권한을 허용한 경우)", "가까운 매장 찾기. 요청 처리에만 쓰고 서버에 저장하지 않습니다")}
${row("모든 방문자(자동 수집)", "쿠키, 브라우저·기기 정보, 방문 페이지, 클릭·스크롤 등 이용 행태", "방문 통계, 화면 개선(6조)")}
</table></div>
<p>비밀번호, 이메일, 전화번호, 결제카드 번호는 받지 않습니다. 카드 정보는 토스페이먼츠가 직접 처리합니다.
AI 대화의 내용은 답변을 만드는 데만 쓰고 서비스 서버에 저장하지 않습니다.
만 14세 미만 아동은 회원으로 가입할 수 없으며, 서비스는 아동의 개인정보를 알면서 수집하지 않습니다.</p>

<h2>2. 민감정보의 처리</h2>
<p>성별·키·몸무게·나이·알레르기는 건강 상태를 짐작할 수 있는 정보이므로 「개인정보 보호법」 제23조의 민감정보로 다룹니다.</p>
<ul>
<li>맞춤 추천 화면에서 <b>별도로 동의한 경우에만</b> 계정(서버)에 저장하고 AI 추천에 사용합니다.</li>
<li>동의하지 않으면 이 정보는 이용자 브라우저에만 남고 서버로 보내지 않습니다. 동의를 거부해도 추천 기능은 그대로 쓸 수 있으며, 다른 기기와의 동기화와 AI 추천에만 반영되지 않습니다.</li>
<li>동의는 맞춤 추천 › 상세 설정에서 언제든 철회할 수 있습니다. 철회하면 서버에 저장된 값과 그 값으로 만든 AI 추천 결과를 즉시 삭제합니다.</li>
<li>서비스는 이용자가 입력한 민감정보를 다른 이용자에게 공개하지 않습니다.</li>
</ul>

<h2>3. 보유 기간과 파기</h2>
<ul>
<li>회원 정보, 추천 설정, 신체정보, 이용 기록, AI 메모리: <b>탈퇴 즉시</b> 삭제합니다. 신체정보는 동의를 철회해도 즉시 삭제합니다.</li>
<li>결제 기록: 「전자상거래 등에서의 소비자보호에 관한 법률」에 따라 대금결제 기록을 <b>5년</b> 보관한 뒤 삭제합니다. 탈퇴하면 회원과의 연결을 끊어 누구의 결제인지 알 수 없게 한 뒤 보관합니다.</li>
<li>결제가 완료되지 않은 주문 기록: 분쟁 확인을 위해 <b>90일</b> 보관한 뒤 삭제합니다.</li>
<li>AI 사용량 기록: 탈퇴 때까지 보관하며, 탈퇴하면 회원과의 연결을 끊어 비용 집계용으로만 남깁니다.</li>
<li>브라우저에 저장된 추천 설정·신체정보·위치·로그인 토큰은 이용자의 기기에만 있으며, 로그아웃하거나 브라우저의 사이트 데이터를 지우면 사라집니다.</li>
</ul>
<p>파기 절차: 보관 기간이 끝난 정보는 매일 자동으로 확인해 삭제합니다. 파기 방법: 전자적 파일은 복구할 수 없도록 데이터베이스에서 삭제하며, 종이 문서로는 개인정보를 보관하지 않습니다.</p>

<h2>4. 제3자 제공</h2>
<p>서비스는 이용자의 개인정보를 제3자에게 제공하거나 판매하지 않습니다. 이용자가 동의하거나 법률에 특별한 규정이 있는 경우(수사기관의 적법한 요청 등)만 예외입니다.
수집 목적과 합리적으로 관련된 범위를 넘는 추가적인 이용·제공도 하지 않습니다.</p>

<h2>5. 처리 위탁과 국외 이전</h2>
<p>서비스 운영을 위해 아래 업체에 개인정보 처리 업무를 위탁합니다. 해외 업체로는 이용자가 해당 기능을 쓰는 시점에 암호화된 네트워크(HTTPS)로 수시로 전송됩니다.
국외 이전은 「개인정보 보호법」 제28조의8 제1항 제3호(계약 이행에 필요한 처리위탁·보관으로서 처리방침에 공개한 경우)에 따릅니다.
국외 이전을 원하지 않으면 로그인·AI 기능을 쓰지 않거나(서버·AI 업체), 브라우저에서 쿠키와 추적을 차단하면(통계 업체) 됩니다. 이 경우 로그인이 필요한 기능과 AI 기능은 쓸 수 없지만, 로그인 없는 기본 기능은 그대로 쓸 수 있습니다.</p>
<div class="wrap"><table>
<tr><th>수탁자·연락처</th><th>국가</th><th>이전 항목</th><th>위탁 업무</th><th>보유 기간</th></tr>
${row(`(주)카카오<br>${ext("https://www.kakao.com/policy/privacy", "처리방침·문의")}`, "대한민국", "카카오 회원번호, 닉네임", "로그인 인증", "카카오 정책에 따름")}
${row(`(주)토스페이먼츠<br>${ext("https://www.tosspayments.com/", "고객센터")}`, "대한민국", "결제 정보", "결제 처리", "관련 법령에 따라 5년")}
${row(`Amazon Web Services, Inc.<br>${ext("https://aws.amazon.com/privacy/", "처리방침·문의")}`, "호주(시드니)", "서비스 서버를 거치는 모든 정보", "서버·웹 호스팅", "처리 즉시 폐기(저장하지 않음)")}
${row(`Neon (Databricks, Inc.)<br>${ext("https://neon.com/privacy-policy", "처리방침·문의")}`, "싱가포르", "1조의 로그인 회원 정보 전부", "데이터베이스 보관", "3조의 기간")}
${row(`Anthropic, PBC<br>${ext("https://www.anthropic.com/legal/privacy", "처리방침")} · privacy@anthropic.com`, "미국", "AI 대화 내용, 목표·칼로리 상한 등 추천 설정, 신체정보(별도 동의한 경우), AI 메모리, 추천 후보 메뉴", "AI 추천·대화 답변 생성(유료 요금제)", "Anthropic API 정책에 따름(모델 학습에 쓰이지 않음)")}
${row(`Google LLC<br>${ext("https://policies.google.com/privacy", "처리방침·문의")}`, "미국", "쿠키, 기기·브라우저 정보, 방문 페이지, 이벤트", "방문 통계(Google Analytics)", "Google Analytics 보관 설정에 따름")}
${row(`Microsoft Corporation<br>${ext("https://privacy.microsoft.com/ko-kr/privacystatement", "처리방침·문의")}`, "미국", "쿠키, 기기·브라우저 정보, 클릭·스크롤·화면 이동", "화면 사용성 분석(Microsoft Clarity)", "Microsoft 정책에 따름")}
${adsense}
</table></div>

<h2>6. 쿠키 등 자동 수집 장치의 설치·운영과 거부</h2>
<p>서비스는 방문 통계(Google Analytics)와 화면 사용성 분석(Microsoft Clarity)을 위해 쿠키를 씁니다.${ADSENSE_CLIENT ? `
광고(Google AdSense)는 Google과 그 파트너가 쿠키를 써서 이용자의 방문 기록(행태정보)에 따라 맞춤 광고를 보여줄 수 있습니다. 맞춤 광고는 ${ext("https://adssettings.google.com", "Google 광고 설정")}에서 끌 수 있습니다.` : ""}
쿠팡 파트너스 등 제휴 링크를 누르면 이동한 사이트가 자체 쿠키를 쓰며, 서비스는 이를 통해 일정액의 수수료를 받을 수 있습니다.
추천 설정, 마지막 위치, 로그인 토큰은 쿠키가 아니라 브라우저의 localStorage에 저장합니다.</p>
<p>거부 방법: 브라우저 설정(예: Chrome › 설정 › 개인정보 및 보안 › 서드파티 쿠키)에서 쿠키를 차단하거나 지울 수 있습니다. 쿠키를 막아도 서비스 이용에는 지장이 없습니다.</p>

<h2>7. 이용자의 권리와 행사 방법</h2>
<p>이용자는 언제든 자신의 개인정보에 대해 열람, 정정·삭제, 처리정지, 동의 철회를 요구할 수 있습니다.</p>
<ul>
<li>추천 설정·신체정보: 맞춤 추천 › 상세 설정에서 확인하고 고치거나 신체정보 동의를 철회할 수 있습니다.</li>
<li>AI 메모리: 목록에서 문장 하나씩 확인하고 지울 수 있습니다.</li>
<li>회원 탈퇴: 내 정보 › 계정 › 회원 탈퇴. 3조에 적은 대로 즉시 삭제됩니다.</li>
<li>그 밖의 요구는 8조의 연락처로 보내 주시면 10일 이내에 처리하고 결과를 알려 드립니다. 대리인을 통해서도 요구할 수 있습니다.</li>
</ul>

<h2>8. 개인정보 보호책임자</h2>
<p>개인정보 보호책임자: ${PRIVACY_OWNER} (운영자)<br>연락처: ${mail}</p>
...
<li>데이터베이스 접속 정보와 API 키는 공개 저장소에 두지 않으며, 운영자 1인만 접근합니다.</li>
<li>AI 대화 내용과 위치 좌표는 저장하지 않고, 신체정보는 별도 동의 없이는 저장하지 않습니다.</li>
</ul>

<h2>10. 가명정보와 자동화된 결정</h2>
<p>서비스는 가명정보를 처리하지 않습니다. AI·알고리즘 추천은 메뉴를 제안할 뿐, 이용자의 권리나 의무에 중대한 영향을 미치는 자동화된 결정(「개인정보 보호법」 제37조의2)을 하지 않습니다.
추천 기준은 <a href="/#about" data-app="about">등급 기준</a>에서 확인할 수 있습니다.</p>

<h2>11. 변경</h2>
<p>이 방침의 내용이 바뀌면 시행 7일 전(이용자에게 불리한 변경은 30일 전)부터 이 페이지에 공지합니다.</p>
<p>공고일 ${PRIVACY_EFFECTIVE} · 시행일 ${PRIVACY_EFFECTIVE}</p>
</div>`,
  }, ctx);
}
