...
${row("<b>신체정보 저장(별도 동의 시에만)</b>", "성별, 키, 몸무게, 나이, 알레르기", "한 끼 적정 열량 계산, 알레르기 메뉴 제외, 기기 간 동기화, AI 맞춤 추천(premium 이용자)")}
${row("서비스 이용(로그인 시)", "메뉴 조회·저장·숨김 기록, 추천에 노출된 메뉴 목록, AI 메모리(취향을 요약한 문장)", "맞춤 추천 개선, 저장한 메뉴 모아 보기")}
${row("AI 대화 이용(premium 이용자)", "날짜별 AI 대화 횟수", "하루 이용 한도 계산")}
...
<h2>3. 보유 기간과 파기</h2>
<ul>
<li>회원 정보, 추천 설정, 신체정보, 이용 기록, AI 메모리, AI 대화 횟수: <b>탈퇴 즉시</b> 삭제합니다. 신체정보는 동의를 철회해도 즉시 삭제합니다.</li>
...
${row(`Anthropic, PBC<br>${ext("https://www.anthropic.com/legal/privacy", "처리방침")} · privacy@anthropic.com`, "미국", "AI 대화 내용, 목표·칼로리 상한 등 추천 설정, 신체정보(별도 동의한 경우), AI 메모리, 추천 후보 메뉴", "AI 추천·대화 답변 생성(premium 이용자)", "Anthropic API 정책에 따름(모델 학습에 쓰이지 않음)")}
...
<li>회원 탈퇴: 8조의 연락처로 요청하시면 3조에 적은 대로 즉시 삭제합니다.</li>
