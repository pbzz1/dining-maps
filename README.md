...
> 프랜차이즈 26곳의 공식 영양정보를 하나의 기준으로 정규화해 비교하고, 내 위치·목표에 맞는 메뉴를 지도에서 바로 찾는 웹 서비스.
> 수집 → 검증 → 적재 → 가공 → 서빙까지 직접 설계·운영한 데이터 엔지니어링 포트폴리오다.
...
로그인 없이 목표(다이어트 / 근성장 / 저나트륨)와 한 끼 상한만 고른다. 신체정보를 넣으면 Mifflin-St Jeor로 하루 필요 열량을 계산해 3끼로 나눈 값이 상한이 된다. 각 메뉴엔 가장 가까운 매장과 거리가 붙는다.
...
외식할 때 주변 메뉴의 영양정보가 브랜드별로 흩어져 있고 형식도 제각각이라 같은 기준으로 비교하기 어렵다. 46개 브랜드를 직접 조사([brand_survey.md](docs/brand_survey.md))하며 확인한 사실이다.
...
공개 형식이 브랜드마다 다르다. 26곳을 뜯어보니 공식 JSON API는 4곳뿐이고 나머지는 HTML을 파싱하거나 아예 사람이 옮겨 적어야 했다.
...
| 빽다방 | 호버 패널 HTML | `/menu/menu_{coffee,drink,dessert}/` | `<table>` 대신 스타일링된 `<ul>`을 써서 범용 표 파서가 놓침. 베스트메뉴 슬라이더와 중복돼 이름 기준 dedup |
...
| 설빙 | 목록→상세 HTML | `/menu/?type=설빙` → `menu_view.php?menu=` | 영양정보가 표 없이 안내문 텍스트로 적혀 있다 — 정규식으로 라벨:숫자 추출 |
| 에그드랍 | 목록→상세 HTML | `/menu/list.php?category=` → `view.php?seq=` | 표마다 `구분` 컬럼 유무가 달라 헤더를 오른쪽 정렬해 맞춤. 인증서 체인 불완전 → SSL 폴백 |
...
브랜드마다 파서는 다르지만 입출력 규약은 `scripts/crawl/crawl_common.py` 하나다.

- `fetch()` — urllib 한 겹. 데스크톱/모바일 UA, POST 자동 전환, 인증서 체인이 깨진 사이트(에그드랍 등)는 SSL 검증 없이 1회 재시도, euc-kr 사이트는 호출부가 인코딩을 지정한다.
- `match_nutrient()` — 라벨→컬럼 매핑을 긴 라벨 우선으로 본다(`포화지방산`을 `지방`보다 먼저). 헤더 텍스트로 컬럼을 찾으니 순서가 바뀌어도 살아남는다.
...
여기서 나온 CSV가 그대로 품질 게이트(`snapshot_and_validate.py`) → 적재(`load_data.py`)로 들어간다. 영양소는 `nutrition_fact` 행으로 풀어 저장한다.
...
100kcal당으로 정규화한다. 100g 기준을 못 쓴 건 샐러디가 중량을 공개하지 않아서다. 한 브랜드라도 정규화가 불가능하면 공정 비교가 깨지므로 모든 브랜드에 있는 칼로리를 분모로 썼다.
...
세 번 갈아엎었다. v1 백분위 → 평균이 항상 50 근처로 수렴해 전 브랜드 C. v2 자체 추정 절대값("225kcal/100g") → 근거가 약함. v3 문헌 인용 절대값이 현재고 v4에서 음료를 분리했다. 음료를 100kcal당으로 재면 "단백질 많은 라떼"가 아메리카노를 이기고 5kcal 음료의 밀도는 의미가 없어서, 음료는 1잔 절대량으로 본다. 100kcal 미만 메뉴는 채점하지 않는다.

등급은 절대·상대 이중이다. 절대 기준(WHO)은 근거가 명확하지만 패스트푸드 특성상 대부분 D로 몰린다. 상대 기준(카탈로그 내 상위 25%씩 A/B/C/D)은 UX에 맞지만 근거가 약하다. 그래서 점수는 절대 기준 그대로 두고 등급 밴드만 상대화해 둘 다 저장한다.
...
브랜드별 "추천 메뉴 + 한 줄 이유"는 Claude structured output으로 생성해 `brand_menu_reco`에 저장한다. 런타임엔 LLM 호출이 없다. 응답 스키마의 `menu_name`을 그 브랜드 실제 메뉴명 enum으로 강제해 없는 메뉴가 나올 수 없고, DB의 `menu_item_id` FK가 이중 안전장치다. 후보는 브랜드당 점수 상위 40개만 넘긴다.

### 매장 위치 — 카카오 API의 45건 상한을 넘는 법

카카오 로컬 검색은 키워드당 최대 45건(15 × 3페이지)만 준다. 전국 2,500개 스타벅스를 한 번에 얻을 수 없어서:

1. 남한을 25km 격자 364개로 나눠 브랜드별로 순회
...
**헤더 텍스트로 컬럼을 찾는다.** `match_nutrient()`가 라벨을 보고 매핑하니 사이트가 컬럼 순서를 바꿔도 살아남는다. 헤더가 중복돼 어쩔 수 없이 인덱스를 고정한 도미노·파파존스는 게이트의 `nutrient_coverage`와 30% 룰이 마지막 방어선이다.

**0행이면 예외** — `write_csv()`는 빈 결과를 쓰지 않는다. 파서가 깨졌을 때 멀쩡한 CSV를 빈 파일로 덮어쓰는 게 최악의 실패 모드라서.

**Airflow가 과한 것 아니냐** — 타당한 지적이었다. 답은 이력 추적이다. 스냅샷을 쌓고 변화를 감지·판정하니 재크롤이 매번 diff를 남긴다. 실제 운영은 GitHub Actions cron이 맡고 Airflow DAG는 같은 흐름을 로컬에서 재현·디버깅하는 용도다.
