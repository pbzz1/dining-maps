...
테두리는 언제나 1px 실선 괘선색이다. 2px 이상의 테두리는 지도 핀의 등급색 좌측 띠(3px), 클러스터 핀의 등급색 원(2px), 목록 등급 배지의 외곽선(1.5px)에만 허용된다 — 지도 위에서 배경과 분리되어야 하고, 등급을 색면 대신 선으로 말하기 때문이다.
...
- **지도 목록 변형**: 카드가 좁아 배지가 유일한 색면이 되는 자리(지도 옆 매장 목록)에서는 면을 채우지 않고 종이빛 면 + 1.5px 등급색 외곽선 + 등급색 글자로 쓴다. 등급 정보가 없으면 뮤티드 점선 외곽선에 `-`.
- 티어 라벨(68px, 반경 14px)은 같은 등급색 면에 24px/700 글자 + 10px 캡션으로, 그 등급이 무엇을 뜻하는지(상위 25% / 80점 이상)를 함께 적는다.

### Map Pin (시그니처)
- **흰 면, 1px 괘선, 등급색 좌측 띠 3px**, `border-radius: 10px 10px 10px 2px`, 잉크색 글자 600/12px, pin 그림자.
- 안에 **종이빛 칩으로 등급 문자**를 등급색 글자로 박는다(색만으로 전달하지 않기 위해).
- 등급색으로 핀 전체를 칠하지 않는다. 첫 화면에 핀이 15개 뜨는데 면을 칠하면 지도가 진한 초록 덩어리가 되고, 정작 브랜드 이름과 로고가 읽히지 않는다. **색은 판단을 가리키는 표시이지 배경이 아니다.**
- 추천 상위 3곳: 골드 1px 테두리 + 등급색 좌측 띠 + 반경 12px + 골드 원형 순위 배지. 이 강조는 상위 3개까지만이며 늘리지 않는다.
- 클러스터: 흰 면에 대표 매장 등급색 2px 원 + 등급색 숫자.
- **영양정보가 없는 주변 식당**: 등급색 없이 괘선 띠 + 뮤티드 이름만. 등급이 있는 핀과 색으로도 형태로도 섞이지 않는다.
- **현 위치**: 로케이터 블루 점 + 흰 2.5px 테두리 + 2초 확산 링. 지도 위에서 파란색은 오직 "나"다.

### Choice Chip (지도 툴바)
- 알약(999px), 흰 면 + 1px 괘선 + 뮤티드 600/12px. 선택되면 소프트 테라코타 면 + 테라코타 테두리·글자.
- 지도 툴바의 첫 줄은 **목표(다이어트·근성장·저나트륨)와 먹고 싶은 것(종류)** 이다. 등급·반경 필터보다 위에 온다 — 사용자가 먼저 말해야 하는 건 조건이 아니라 무엇을 먹을지다.
