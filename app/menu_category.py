...
GROUP_KEYWORDS = (
    ...
    # '불고기'도 뺐다 -- 버거가 아니라 양념 이름이라 불고기 샌드위치·샐러드·비빔밥·볶음밥 12종이
    # 버거로 잡혔다. 불고기버거·불고기와퍼는 '버거'/'와퍼'로 그대로 걸린다(2026-09-21 DB 전수 확인:
    # '불고기' 하나로만 버거가 되던 12종 중 실제 버거는 0).
    ("버거", ("버거", "와퍼", "빅맥", "맥스파이시", "맥치킨", "맥크리스피",
...
if __name__ == "__main__":
    ...
    # '불고기'는 양념이지 버거가 아니다 (예전 키워드라 샌드위치·밥까지 버거로 잡혔다)
    assert category_group("랩&샌드위치", "불고기 반미 샌드위치") == "샐러드·샌드위치"
    assert category_group(None, "불고기 샐러드") == "샐러드·샌드위치"
    assert category_group("푸드", "현미 소불고기볶음밥") != "버거"
    assert category_group("비빔밥", "소불고기 감초고추장 비빔밥") != "버거"
    assert category_group("리아모닝", "야채라이스 불고기") != "버거"
    # 진짜 불고기 버거는 그대로 버거
    assert category_group("버거메뉴", "리아 불고기") == "버거"
    assert category_group(None, "불고기버거") == "버거"
    assert category_group("와퍼", "불고기와퍼") == "버거"
    print("ok")
